...
for coin in coin_list:
    ...
    if mode == 5 or mode == 6:
        df, adx, adx_current_signal= Adx.calculate_adx(coin, 'minute5', 14)
        plus_di = df['plus_di']
        minus_di = df['minus_di']
        Adx.adx_grph(coin, df, adx, plus_di, minus_di, adx_current_signal, threshold=25)

        if mode == 6:
            progress += (1 / 4)
        print("[", round(progress / 30 * 100), "%] ADX Done on coin ", coin)
