...
def rsi_grph(coin_name, df, recent_hours=6):
    df, rsi, buy_signals, sell_signals, current_signal = rsi_calc(coin_name, df)
...
def backtest_rsi(df):
    ...
    return final_balance, trade_signals

# 가상의 가격 데이터 생성
np.random.seed(0)
date_rng = pd.date_range(start=datetime.now() - timedelta(days=365), end=datetime.now(), freq='H')
price_data = np.random.uniform(100, 200, size=(len(date_rng),))
df = pd.DataFrame(price_data, columns=['close'], index=date_rng)

# 백테스트 및 그래프 그리기
final_balance, trade_signals = backtest_rsi(df)
print("Final Balance:", final_balance)

for signal, timestamp in trade_signals:
    print(f"Signal: {signal}, Timestamp: {timestamp}")
